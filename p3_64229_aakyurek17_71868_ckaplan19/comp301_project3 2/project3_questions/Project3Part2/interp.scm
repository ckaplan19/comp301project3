  ...
  (define value-of
    (lambda (exp env)
      (cases expression exp
        ...
        (letrec-nested-exp (p-name b-var count p-body letrec-body)
          (value-of letrec-body
  ...
  (define apply-procedure
    (lambda (proc1 arg)
      (cases proc proc1
        ...
        ; #####################################################
        (nested-procedure (bvar body saved-env count name)
                          (begin
                            (recursive-displayer name count)
                            (value-of body (extend-env-rec-nested name bvar body (extend-env bvar arg saved-env) (+ count 1)))
                            )
                          )
