      ...
      (cases environment env
        ...
        (extend-env (var val saved-env)
          (if (eqv? search-sym var)
              ...
              (if (proc? (expval->all val))
              (cases proc (expval->all val)
                    (nested-procedure (bvar body env count name)       
                      (proc-val (nested-procedure bvar body env count name))
                    )
                    (else val)
                  )
              (if (number? (expval->all val))(num-val (expval->all val))
                  (bool-val (expval->all val)))
              )
        ...
        ; ###### for the nested procedures.
         ...
         (extend-env-rec-nested (id bvar body saved-env count)
                                (if (eqv? search-sym id)
                                    (proc-val (nested-procedure bvar body saved-env count id))
                                    (apply-env saved-env search-sym)))
